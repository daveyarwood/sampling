...
app.post("/api/auth/exchange-code", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to exchange code for token.", error: message });
  }
});
...
app.get("/api/random-samples", async (_req: Request, res: Response) => {
  if (!hasToken()) {
    return res
      .status(401)
      .json({ message: "Authentication required. Please set up the application." });
  }
  ...
  try {
    ...
    if (axios.isAxiosError(error) && error.response?.status === 401) {
      try {
        ...
        return res
          .status(500)
          .json({ message: "Failed to refresh token. You may need to re-authenticate." });
      }
    }
    const message = error instanceof Error ? error.message : "An unknown error occurred";
...
app.get("/api/download-zip", async (_req: Request, res: Response) => {
  ...
  try {
    const files = await fsPromises.readdir(samplesDir);
